...
chunk_log_fields = OrderedDict(
    [("chunk_number", "INTEGER"), ("chunk_udprn", "INTEGER"), ("chunk_umrrn", "INTEGER")]
)
...
def build_cache(constructors, cache_db, cache_fields, sha, chunk_size=1000, test=True):
    ...
    for id_, (key_generator, key_count, make_row_method) in enumerate(constructors):
        ...
        line_count = updater(
            id_, key_generator, key_count, make_row_method, cache_db, db_fields, sha, chunk_size
        )
        #
...
def updater(id_, key_method, get_key_count, make_row_method, cache_db, db_fields, sha, chunk_size):
    ...
    print(
        "{} chunks will be skipped because they've been done already".format(chunk_skip), flush=True
    )
    ...
    for i, keys in enumerate(key_chunks):
        ...
        logger.info(
            "Dispatching chunk {0}, commencing ({1}, {2}) at {3:.1f} seconds".format(
                i, chunk_id_udprn, chunk_id_umrrn, elapsed
            )
        )
    ...
    for future in as_completed(futures):
        ...
        time_ = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        update_to_db(
            [(n_chunks_received + chunk_skip, time_, sessid)],
            cache_db,
